...
class UserManager:
    ...
    def update_user(uid, **kwargs):
        ...
        try:
            print(f"Updating user {uid} with data: {kwargs}")
            for key, value in kwargs.items():
                if key == "password":
            ...
            db.session.commit()
            print(f"User {uid} updated successfully. New mobile value: {user.mobile}")
            ...
            db.session.rollback()
            print(f"Error updating user {uid}: {str(e)}")
            return {"error": str(e)}, 500
...
class DonationManager:
    @staticmethod
    def add_donation(user_id, items=None, warehouse_id=None, phone=None, amount=None):
        """
        ...
        try:
            ...
            new_donation = Donation(user_id=user_id, items=items, warehouse_id=warehouse_id, phone=phone, amount=amount)
            db.session.add(new_donation)
            db.session.commit()
            return {"message": "Donation added successfully", "donation_id": new_donation.did}
        except ValueError as e:
            db.session.rollback()
            print(f"Validation error in add_donation: {str(e)}")
            raise
        except Exception as e:
            db.session.rollback()
    ...
    def add_donation_amount(user_id, amount):
        ...
        """
        # Get the next amount_id for this user
        last_donation = DonationAmount.query.filter_by(user_id=user_id).order_by(DonationAmount.amount_id.desc()).first()
        next_amount_id = 1 if last_donation is None else last_donation.amount_id + 1
        
        new_donation_amount = DonationAmount(user_id=user_id, amount_id=next_amount_id, amount=amount)
        db.session.add(new_donation_amount)
        db.session.commit()
        return {"message": "Donation amount added successfully", "donation_amount_id": new_donation_amount.amount_id}
    ...
    def get_donation_by_user(user_id):
        ...
        try:
            ...
            result = []
            for donation in donations:
                if donation.items:  # Check if items exist
                    # Handle items as JSON data
                    if isinstance(donation.items, list):
                    ...
                    elif isinstance(donation.items, dict):
                        # Handle case where items might be a dictionary
                        for name, quantity in donation.items.items():
                            result.append([name, quantity])
            
            return result
        except Exception as e:
            print(f"Error in get_donation_by_user: {str(e)}")
    ...
    def get_donation_amount_by_user(user_id):
        ...
        try:
            ...
            return 0  # Return 0 if no donation amount found
        except Exception as e:
            print(f"Error in get_donation_amount_by_user: {str(e)}")
