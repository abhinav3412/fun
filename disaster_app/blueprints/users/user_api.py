...
def donate_amount():
    ...
    result = DonationManager.add_donation_amount(user_id=current_user.uid, amount=amount)
    # Return a success response
    return jsonify({"success": True, "message": result["message"], "donation_amount_id": result["donation_amount_id"]}), 200
...
def donate_items():
    try:
        ...
        if not warehouse_id:
            return jsonify({"success": False, "error": "Warehouse selection is required"}), 400
        ...
        # Update user's location with the provided address
        current_user.location = address
        db.session.commit()

        # Save the donation using the DonationManager
        result = DonationManager.add_donation(user_id=current_user.uid, items=items, warehouse_id=warehouse_id, phone=phone)
        ...
        print(f"Error in donate_items: {str(e)}")
        return jsonify({"success": False, "error": "An error occurred while processing your donation"}), 500
...
def payment_success():
    ...
    if verification:
        ...
        user_id = current_user.uid  # Get the current user's ID
        result = DonationManager.add_donation_amount(user_id=user_id, amount=amount_paid)

        return jsonify({
            "success": True, 
            "message": "Payment successful", 
            "amount_paid": amount_paid,
            "donation_amount_id": result["donation_amount_id"]
        })
    else:
...
def get_user_donation_summary():
    try:
        ...
        print(f"Error in user-donation-summary: {str(e)}")
        return jsonify({
            "success": False,
...
@user_bp.route('/send_alert', methods=['POST'])
def send_alert():
    try:
        ...
        print(f"Received alert request - Location: {location}, Time: {predicted_time}, Status: {status}")
        
        if not all([location, predicted_time, status]):
            print("Missing required fields in alert request")
            return jsonify({'error': 'Missing required fields'}), 400
            
        # Send SMS alerts to all users
        print("Attempting to send SMS alerts to all users...")
        success = send_alert_to_users(location, predicted_time, status)
        
        if success:
            print("Alerts sent successfully")
            print("===== ALERT REQUEST COMPLETED =====\n")
            return jsonify({'message': 'Alerts sent successfully'}), 200
        else:
            print("Failed to send alerts")
            print("===== ALERT REQUEST FAILED =====\n")
            return jsonify({'error': 'Failed to send alerts'}), 500
            
    except Exception as e:
        print(f"Error in send_alert: {str(e)}")
        print("===== ALERT REQUEST ERROR =====\n")
        return jsonify({'error': str(e)}), 500
