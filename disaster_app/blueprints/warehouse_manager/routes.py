...
@warehouse_manager_bp.route('/get_donations')
@login_required
@warehouse_manager_required
def get_donations():
    try:
        # Get the warehouse manager's warehouse
        warehouse = Warehouse.query.filter_by(manager_id=current_user.uid).first()
        if not warehouse:
            return jsonify({'success': False, 'error': 'No warehouse assigned'}), 400

        # Get all donations for this warehouse
        donations = Donation.query.filter_by(warehouse_id=warehouse.wid).order_by(Donation.timestamp.desc()).all()
        
        # Format donations for response
        formatted_donations = [{
            'did': donation.did,
            'donor_name': donation.user.username if donation.user else 'Unknown',
            'phone': donation.phone,
            'items': donation.items,
            'timestamp': donation.timestamp.isoformat() if donation.timestamp else None,
...
@warehouse_manager_bp.route('/handle_donation', methods=['POST'])
@login_required
@warehouse_manager_required
def handle_donation():
    try:
        ...
        if not donation_id or not action:
        ...
        # Verify the donation belongs to the warehouse manager's warehouse
        warehouse = Warehouse.query.filter_by(manager_id=current_user.uid).first()
        if not warehouse or donation.warehouse_id != warehouse.wid:
        ...
        current_app.logger.error(f"Error handling donation: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500

@warehouse_manager_bp.route('/download_donations')
@login_required
@warehouse_manager_required
def download_donations():
    try:
        ...
        # Get all donations for this warehouse
        donations = Donation.query.filter_by(warehouse_id=warehouse.wid).all()
        ...
        # Write header
        writer.writerow(['Donation ID', 'Donor Name', 'Phone', 'Address', 'Items', 'Timestamp', 'Status'])
        
        # Write data rows
        for donation in donations:
            try:
                items = json.loads(donation.items) if isinstance(donation.items, str) else donation.items
                items_str = ', '.join([f"{item['name']} ({item['quantity']}) - {item['condition']}" for item in items])
            except:
                items_str = 'Error formatting items'

            # Debug logging
            current_app.logger.info(f"Processing donation {donation.did}:")
            current_app.logger.info(f"User: {donation.user.username if donation.user else 'Unknown'}")
            current_app.logger.info(f"Location: {donation.user.location if donation.user else 'N/A'}")

            writer.writerow([
                str(donation.did),
                donation.user.username if donation.user else 'Unknown',
                donation.phone,
                donation.user.location if donation.user else 'N/A',
                items_str,
                donation.timestamp.strftime('%Y-%m-%d %H:%M:%S'),
                donation.status
            ])
        
        # Convert the string buffer to bytes with UTF-8 encoding
        output = BytesIO()
        output.write(string_buffer.getvalue().encode('utf-8-sig'))  # Using UTF-8 with BOM for Excel compatibility
        output.seek(0)
        
        # Generate filename with current date
        filename = f'donations_{datetime.now().strftime("%Y%m%d")}.csv'
        ...
        current_app.logger.error(f"Error generating donations CSV: {str(e)}")
        return jsonify({'error': str(e)}), 500
