...
class VolunteerHistory(db.Model):
    ...
    vid = db.Column(
        db.Integer, 
        db.ForeignKey('volunteers.id', name='fk_volunteer_history_volunteer_id', ondelete='SET NULL'),
        nullable=False
    )
    camp_id = db.Column(
        db.Integer, 
        db.ForeignKey('camps.cid', name='fk_volunteer_history_camp_cid', ondelete='SET NULL'),
        nullable=False
    )
    role_id = db.Column(
        db.String, 
    ...
    def __repr__(self):
        return f"<VolunteerHistory {self.vhid}>"
...
class Donation(db.Model):
    ...
    warehouse_id = db.Column(
        db.Integer,
        db.ForeignKey('warehouses.wid', name='fk_donation_warehouse_id'),
        nullable=False
    )  # Warehouse where the donation will be stored
    ...
    status = db.Column(db.String(20), nullable=False, default='Pending')  # Status of the donation

    # Relationships
    user = db.relationship("User", back_populates="donation")
    warehouse = db.relationship("Warehouse", backref="donations")

    def __init__(self, user_id, amount=None, items=None, warehouse_id=None, phone=None):
        """
        ...
        :param warehouse_id: ID of the warehouse where items will be stored.
        :param phone: Phone number for contact (optional).
        """
        if not user_id:
        ...
        self.phone = phone
        self.status = 'Pending'  # Set initial status
        
        # Handle items or amount
        if items is not None:
            self.items = items
        elif amount is not None:
            # If amount is provided, create a DonationAmount record
            # We don't store amount in this model, it's stored in DonationAmount
            self.items = []  # Default to empty items
        else:
            # If neither is provided, default to empty items
            self.items = []

    def __repr__(self):
        return f"<Donation(did={self.did}, user_id={self.user_id}, items={self.items}, warehouse_id={self.warehouse_id}, phone={self.phone}, status={self.status})>"
