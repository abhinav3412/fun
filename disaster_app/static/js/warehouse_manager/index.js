...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupEventListeners() {
    ...
    // Add donations refresh to the periodic updates
    setInterval(() => {
        fetchDonations();
    ...
    const downloadBtn = document.getElementById('download-donations');
    if (downloadBtn) {
        downloadBtn.addEventListener('click', downloadDonations);
    }
  }
  ...
  async function fetchDonations() {
    try {
        const response = await fetch('/warehouse_manager/get_donations');
  ...
  function updateDonationsTable(donations) {
    ...
    pendingDonations.forEach(donation => {
        ...
        row.innerHTML = `
            ...
            <td>${formatItems(donation.items)}</td>
            <td>${new Date(donation.timestamp).toLocaleString()}</td>
            <td>
                <span class="status-badge status-${donation.status.toLowerCase()}">
                    ${donation.status}
                </span>
            </td>
            <td>
                <button class="action-btn complete-btn" onclick="handleDonation(${donation.did}, 'complete')">Mark as Complete</button>
            </td>
        `;
        tableBody.appendChild(row);
    });
  ...
  function formatItems(items) {
    ...
    try {
        // Handle case where items is already a JavaScript object
        const itemsList = typeof items === 'string' ? JSON.parse(items) : items;
        if (!Array.isArray(itemsList)) {
            console.error('Items is not an array:', itemsList);
        ...
        console.error('Error formatting items:', error, 'Items:', items);
        return 'Error formatting items';
    }
  }

  // Add handleDonation to window object
  window.handleDonation = async function(donationId, action) {
    try {
        const response = await fetch('/warehouse_manager/handle_donation', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (!response.ok) throw new Error('Failed to handle donation');
        
        const data = await response.json();
        if (data.success) {
            alert('Donation marked as complete');
            fetchDonations(); // Refresh the donations table
        } else {
  ...
  // Add download donations function
  async function downloadDonations() {
    try {
        ...
        const response = await fetch('/warehouse_manager/download_donations');
        
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Failed to download donations');
        }
        ...
        if (downloadBtn) {
            ...
            downloadBtn.textContent = 'Download Donations CSV';
        }
    } catch (error) {
        console.error('Error downloading donations:', error);
        alert(`Error downloading donations: ${error.message}`);
