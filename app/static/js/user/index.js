...
async function getCurrentUser() {
    try {
        const response = await fetch('/user/get_current_user');
        if (!response.ok) {
...
async function addPeopleToCamp(campId) {
    try {
        ...
        // First, check if the user is already in the camp
        const checkResponse = await fetch(`/user/people-list/${campId}`);
        if (!checkResponse.ok) {
            throw new Error('Failed to check if user is already in camp');
        }
        
        const peopleList = await checkResponse.json();
        const userAlreadyInCamp = peopleList.some(person => person.uid === currentUser.uid);
        
        if (userAlreadyInCamp) {
            console.log('User is already in this camp');
            return; // Exit early if user is already in the camp
        }
        ...
        for (const person of people) {
            const response = await fetch(`/user/add_person_to_camp/${campId}`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                if (errorData.error && errorData.error.includes('already exists')) {
                    console.log('Person already exists in camp, skipping...');
                    continue; // Skip to the next person if this one already exists
                }
                throw new Error(errorData.error || 'Failed to add person to camp');
            }
        }
