...
let previousSensorStates = {};

// Function to initialize alert sound
function initializeAlertSound() {
    if (!alertSound) {
        alertSound = new Audio('/static/sounds/alert-sound.mp3');
        ...
        // Add event listener for when sound is ready
...
// Function to safely play alert sound
async function playAlertSound() {
    if (!alertSound) {
        console.log('Alert sound not initialized or failed to load');
        // Create a simple beep sound as fallback
        try {
    ...
    try {
        ...
        if (error.name === 'NotAllowedError') {
            console.log('Sound playback not allowed. Will try again on next user interaction.');
            // Add a one-time click listener to play the sound on next user interaction
...
// Initialize alert sound immediately
initializeAlertSound();

// Add debug logging
console.log('Alert.js loaded');

// Add periodic sensor data update
setInterval(async () => {
    ...
    if (sensors && sensors.length > 0) {
        ...
        updateAlerts(sensors, userLocationCircle.getLatLng().lat, userLocationCircle.getLatLng().lng);
    } else {
        console.log('No sensors found or error fetching sensors');
    }
...
function updateAlerts(sensors, userLat, userLng) {
    ...
    sensors.forEach(sensor => {
        ...
        if (sensor.operational_status !== 'Active') {
            console.log(`Skipping inactive sensor: ${sensor.name}`);
            return;
        }
        
        console.log(`Processing sensor: ${sensor.name}, Status: ${sensor.status}`);
        
        if (sensor.status === 'Alert' || sensor.status === 'Warning') {
            console.log(`Alert/Warning detected for sensor: ${sensor.name}`);
            ...
            // Check if status has changed to Warning or Alert
            const previousState = previousSensorStates[sensor.name] || 'Normal';
            console.log(`Sensor ${sensor.name} status changed from ${previousState} to ${sensor.status}`);

            // Send alert if status has changed to Warning or Alert
            if ((sensor.status === 'Warning' || sensor.status === 'Alert') && 
                (previousState !== 'Warning' && previousState !== 'Alert')) {
                console.log('Alert status change detected, sending alert...');
                ...
                fetch('/user/send_alert', {
                    ...
                    body: JSON.stringify({
                        ...
                        predicted_time: sensor.predicted_landslide_time,
                ...
                .then(data => {
                    ...
                    successMessage.textContent = data.message || 'Alert sent successfully to all users';
                    alertBox.appendChild(successMessage);
                ...
                .catch(error => {
                    ...
                    errorMessage.textContent = error.message || 'Failed to send alert. Please try again.';
                    alertBox.appendChild(errorMessage);
                    setTimeout(() => errorMessage.remove(), 5000);
                });
            } else if (sensor.status === 'Alert' || sensor.status === 'Warning') {
                // Play alert sound for existing alerts/warnings too
                playAlertSound();
            }
        ...
        previousSensorStates[sensor.name] = sensor.status;
    });
}
