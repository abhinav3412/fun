...
from app.models import User, Warehouse, Camp, UserActivity, Donation
...
def update_user(uid):
    ...
    try:
        ...
        # Debug logging for mobile field
        print(f"Received update data: {update_data}")
        if 'mobile' in update_data:
            print(f"Mobile field value: {update_data['mobile']}")
            print(f"Mobile field type: {type(update_data['mobile'])}")
        ...
        if uid == current_user.uid:
            log_recent_activity(user_id=current_user.uid, action=f"Updated own account details")
        else:
            log_recent_activity(user_id=current_user.uid, action=f"Updated user: {user.username} (ID: {uid})")
...
def delete_user(uid):
    ...
    try:
        ...
        # Only check for admin if not force deleting
        if not force_delete:
            ...
            donations = Donation.query.filter_by(user_id=uid).all()
            if donations:
                return jsonify({
                    'error': f'Cannot delete user because they have {len(donations)} donations associated with their account. Please reassign or delete the donations first.'
                }), 400
        ...
        try:
            # If force deleting, first delete all related records
            if force_delete:
                # Delete all donations
                Donation.query.filter_by(user_id=uid).delete()
                # Delete all activities
                UserActivity.query.filter_by(user_id=uid).delete()
                # Remove warehouse manager assignments
                Warehouse.query.filter_by(manager_id=uid).update({'manager_id': None})
                # Remove camp manager assignments
                Camp.query.filter_by(camp_head_id=uid).update({'camp_head_id': None})
                
            # Delete user
            db.session.delete(user)
            db.session.commit()
            
            action = "Force deleted" if force_delete else "Deleted"
            log_recent_activity(user_id=current_user.uid, action=f"{action} user: {username} (ID: {uid})")
            return jsonify({'message': f'User {username} {action.lower()} successfully'}), 200
            
        except Exception as db_error:
            db.session.rollback()
            if not force_delete and 'foreign key constraint' in str(db_error).lower():
                return jsonify({
                    'error': 'Cannot delete user because they have related records. Use force delete to remove all related records.'
                }), 400
            raise db_error
